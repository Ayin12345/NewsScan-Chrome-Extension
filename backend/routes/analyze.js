...
export async function analyzeRoute(req, res) {
  try {
    ...
    if (failedProviders.length > 0) {
      failedProviders.forEach(failedProvider => {
        logger.warn(`[Analyze] Provider ${failedProvider.provider} failed:`, {
          error: failedProvider.error,
          errorCode: failedProvider.errorCode,
          details: failedProvider.details
        });
      });
    }
